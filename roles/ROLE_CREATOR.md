...
> **You also supply the ONE input the load analysis cannot invent: the expected scale.** How many users, how concentrated their peak, how fast the data grows — these are *product* facts that live with you, @BIZ and @DOMAIN_EXPERT, not with @ARCH. Put them in `PROJECT_PROFILE.md` §1 ("Expected scale") and in the package. **You do not run `SYSTEM_DESIGN_PROTOCOL`** — that is @ARCH's, and it happens after your package. But an @ARCH that has to guess the scale designs against a number nobody chose, and that number is usually one user.
