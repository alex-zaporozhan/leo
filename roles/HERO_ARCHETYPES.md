...
> This document closes the gap that caused the hero to always converge to a single split-with-mockup SaaS template: the `@DESIGN` golden library was a density monoculture of a few named SaaS products, and composition archetypes were not described.
...
**Where this archetype fits:** dense, operational product pages where the interface itself is the proof.
...
**Where it fits:** a brand or platform page that must read as serious and settled before it reads as clever.
...
**Where it fits:** a visual product whose interface is itself the argument.
...
**Where it fits:** brand landings, premium services, editorial and fashion formats (as a composition reference, not as content).
...
**Where it fits:** infrastructure and fintech, where the background carries the sense of a living system.
...
**Where it fits:** a multi-faceted product that has several equal things to say at once.
...
| Archetype | Product nature | Ambition | Feeling | The gesture it commits to |
|---------|----------------|---------|---------|--------|
| A Split + mockup | operational/data | restrained–confident | "clear, reliable" | the product's own UI carries the proof |
| B Centered Statement | brand/platform | restrained–confident | "I trust, serious" | one sentence, centred, with room around it |
| C Full-Bleed Type | creative/portfolio | bold–experimental | "character, bold" | type at display scale IS the image |
| D Product-Immersive | visual product | confident–bold | "wow" | the interface fills the frame and moves |
| E 3D / WebGL | spatial/AI | bold–experimental | "immersion" | a spatial scene the visitor can disturb |
| F Editorial | content/premium | bold | "taste, story" | an asymmetric spread that reads like print |
| G Background-Driven | infra/fintech | confident | "living system" | the background is alive; the foreground is calm |
| H Bento / Modular | multi-faceted product | confident | "rich, modern" | several equal claims in one grid |
