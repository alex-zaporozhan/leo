...
**v6.40b — the composition floor, merged into `INTERFACE_CRAFT_CANON` §3.5; `SCREEN_FLOORS_CANON.md` withdrawn.**
The v6.40 diagnosis held and the delivery did not. A clean-context audit read the new file as a router entry rather than as prose, and reported four structural faults — each of which is a rule this system already had:
- **It displaced an existing canon from a class minimum.** `TC-01` READ is a six-file budget; the new file took slot 5 and `COMPONENT_REGISTRY` fell out. Law 25 step 4 sends @DESIGN to the registry before proposing any component, so the class that builds screens lost the file that says which components exist. `RAG_CANON` §2: **a class over budget is trimmed by naming sections, never by dropping a canon silently.**
- **It restated owned decisions with different values.** Row height, focus ring, spacing steps and motion durations already have homes in `VISUAL_CRAFT_CANON` §2/§3/§5 and `MOTION_CRAFT_CANON` §1. The copies were not marked as echoes and did not agree. `RULE_INTEGRITY_PROTOCOL` T2 permits a full echo **only where it names its home**; an unowned copy is the forbidden form, and a disagreeing unowned copy is the one that decides at the keyboard.
- **It silently deprecated five rules it did not own** through an invented *"Retires:"* line — including Law 8 and M10 of a blocking catalogue, belonging to four other files. Nothing in the system grants a canon the power to retire another file's rule, least of all a law.
- **It said "six states" over an eight-line block.** Two of the six were genuinely new; four were restatements of the four already owned by `ROLE_DESIGN`'s State Spec, which is the registered SoT for states.

**What was done instead:**
- **`INTERFACE_CRAFT_CANON` §3.5 — THE COMPOSITION FLOOR.** The compositional half survives in full and lands where composition is already owned: list/table, form, record, filter bar, destructive action, each stated as the construction to take rather than the ban to avoid. **No value that §2/§3/§5 already sets is repeated**, and states are explicitly not restated — the section names their owner instead.
- **The two real states moved to their owner.** `ROLE_DESIGN`'s State Spec gained **loading-on-REFETCH** (content stays, dimmed; scroll and selection survive) and **error-that-retry-cannot-fix** (403/404/validation — **no Retry button**), and filtered-empty was sharpened (no create-CTA, `[Clear filters]`, "0 of 4,102"). One home, four states plus a named intermediate list, no second copy.
- **`ROLE_QA_ARCH` Vector 3 rewritten** to grade the four base states **and the intermediate list** — the enforcement half, which the withdrawn file never had. This is why no new detector series was opened: see below.
- **The class minimums restored.** `COMPONENT_REGISTRY` back at `TC-01` READ item 5, `FRONTEND_CONSOLIDATION` back at `TC-20` item 7.
- **The third floor still stands and now says so from inside a canon that was already routed:** tokens (`VISUAL_CRAFT_CANON` §11) · movement (`MOTION_CRAFT_CANON` §1) · composition (`INTERFACE_CRAFT_CANON` §3.5). A screen that took one of the three took a third of the floor.
- Drift cleared in the same pass: `ROLE_DEV`'s broken blockquote table row, and its Type-C line demanding a confirm on *every* destructive action, which contradicted undo-by-default.

**On a `SC1–SC12` series for the composition floor — deliberately not created.** The four-part test (`ROLE_LEO_EDITOR` §4) asks for a floor, detectors, a reflex and a blocking vector. Composition has all four already, under other names: **`ST1–ST12`** (interaction stiffness) covers roughly five of the constructions directly, **@QA_ARCH Vector 19** carries twenty checks over exactly these surfaces (19.5 partial failure, 19.13 skeleton, 19.15 bulk partial, 19.19 filter persistence), and **Vector 3** now grades the state list. A new letter series would restate those checks under a second name, produce two verdicts for one defect, and collide with the reused-letter list `RAG_CANON` §1.1 had just been corrected to keep unambiguous. **The gap was enforcement of the states, not a missing series — and that was closed in Vector 3.**

**Completion rule (`ROLE_LEO_EDITOR` §5):**
```
CLAIM:   the withdrawn file is unreachable from anywhere but this log
GREP:    rg -n "SCREEN_FLOORS" .cursorrules roles/ *.md
OUTPUT:  only roles/SYSTEM_UPGRADE_MANIFEST.md (v6.40 and v6.40b entries)

CLAIM:   the displaced canons are back in their class minimums
GREP:    rg -n "COMPONENT_REGISTRY|FRONTEND_CONSOLIDATION" roles/RAG_CANON.md
OUTPUT:  TC-01 READ item 5 = COMPONENT_REGISTRY; TC-20 READ item 7 = FRONTEND_CONSOLIDATION

CLAIM:   no live rule claims six states (this log keeps the history)
GREP:    rg -ni "six states" .cursorrules roles/ *.md -g '!SYSTEM_UPGRADE_MANIFEST.md'
OUTPUT:  (empty)

CLAIM:   no dead paths
GREP:    grep -rhoP 'roles/[A-Za-z0-9_/]+\.md' .cursorrules roles/ *.md | sort -u | while read x; do [ -f "$x" ] || echo BROKEN $x; done
OUTPUT:  (empty)
```

**v6.40 — `SCREEN_FLOORS_CANON.md` (withdrawn — see v6.40b): the design layer gets its affirmative half.**
Measured first, then built. The design canons carry **9.1 prohibitions per 10k characters against the backend layer's 4.6**, at the same density of verifiable thresholds — twice the bans for the same rigour. The cause is structural, not sloppy: **a backend ban states the construction in the same sentence** ("every outgoing call has a timeout" also tells you what to type), and a design ban does not — *"never two separation methods on one surface"* never says which one to use. So the construction had to be stated separately, and mostly it never was. Law 5 already names what is left: **a warning with no call to action is noise.**
- **The third floor.** Tokens (`VISUAL_CRAFT_CANON` §11) · movement (`MOTION_CRAFT_CANON` §1) · **composition (this file)** — taken together, and each of the three now says so. A screen that took only the token floor took a third of it.
- **§1 — THE SIX STATES, and there are six, not four.** Every canon in the system said "loading · empty · error · success", and **three of those four hide a second state that is always the one that ships broken**: loading-on-refetch (the table blanks and the reader loses their place, instead of dimming and keeping scroll and selection) · empty-because-filtered ("create your first client" over 4,102 clients behind a filter — different words, different action: *Clear filters*, not *Create*) · error-that-retry-cannot-fix (an infinite Retry on a 403) · **success-that-was-partial** ("Imported successfully" over 12 of 400 rows). This one section is the highest-value part of the wave: it is the difference between a checkbox that has been ticked on every project and a defect that shipped on every project.
- **§2–§6 — the take-this constructions** for the surfaces an admin product is made of: the list/table (row height, the identifier column, status on the leading edge, one visible action, the default sort stated in the spec rather than left to the ORM, "1–50 of 4,102") · the form (labels above, mark the *optional* ones, validation on blur, **never a dead disabled submit** — say what is wrong instead of going silent, unsaved handled once) · the record (one number that matters, tabs only for genuinely independent sections, related tables as their own lists) · filter and search (an applied filter visible as a removable chip, the "N of M" line that prevents the most common support ticket in any admin product, filters in the URL, **and every offered filter column indexed — cannot be indexed, not offered**, which is `LOAD_REFLEX` LD3 finally stated as a construction) · the destructive action (undo by default, confirm reserved for the irreversible and then naming the object).
- **Every floor names the ban it retires.** The file adds no new rules — it is the affirmative half of rules that already exist, and once the construction is named the ban stops having to do the work alone.
- **And it says what it does not decide:** the world, the register, the tokens, the motion values, the business minimum per page type, the load ceilings. Composition only, overridden by any of those where they speak — and it is a floor, not a ceiling: `PRODUCT_MATURITY_CANON` grades taking the floor everywhere as Level 1, correct and unremarkable.
- Routed as READ item 5 of `TC-01`, wired into @DESIGN, @FRONTEND and @DEV activation, and `DOMAIN_STANDARDS` §9's four-line State Matrix rewritten to six with the construction pointing here. Law 19's skip-list now has a written definition of "an existing pattern".

**v6.39d — the load wave audited in a clean context, and it had installed the gate one level too deep.**
A clean-context pass traced two concrete modules through the system and reported: *the discipline fires reliably in exactly one shape — a project start or a store change — and on the two triggers that generate most load defects it is never asked.*
- 🔴 **The gate was a bullet inside @LEAD step 1.8**, whose own question is "an architectural trigger: new service / store / integration / queue / schema / public contract / tier?". **The load trigger list is wider.** A refund posting on an existing schema and a sortable, searchable list over an existing table are load decisions and *none* of them is a new service or contract — so 1.8 answered "no" and the load block was never read. **Now step 1.75, its own question, asked before 1.8**, with the reason stated in the step: it does not look like architecture, and it is the only failure class whose symptom is success. Law 31 says the same.
- 🔴 **The @QA_ARCH mirror pointed at vectors that do not exist** — "Vector 17 and the Perf vector"; 17 is database runtime and there is no Perf vector. So the reflex reached @DEV, and at the other end there was no vector to run it under, no verdict rule and no report cell: 🟢 could be issued without checking and that violated nothing written. **New Vector 20 (Load)** — the load profile as its input, LD1–LD12 as its checks, and a verdict: any surviving **LD1/LD2/LD5/LD6/LD8** is 🔴 on its own, **3+ hits across the twelve is 🔴** whatever they are — the missing threshold that `ROLE_LEO_EDITOR` §4 requires of any detector set.
- 🔴 **Four of the twelve greps did not match this stack, two failing on their own printed examples.** `response_model=List\[` misses `list[ClientOut]` on the next line; `def (get|post|…)_` misses `async def build(...)`, its own example; `.filter(` is the SQLAlchemy **1.x** Query API — SQLAlchemy 2 writes `.where()`, so a filter on an unindexed column, LD3's whole point, was missed; several patterns are multiline and `grep` is line-based, so they returned nothing silently. Rewritten as `rg` with `-U`/`-P` where needed, and the file now says so at the top: **a reflex that silently finds nothing is worse than no reflex.**
- 🟠 **"Every hit is a stop" made the reflex a ritual.** A correctly paginated keyset endpoint contains `.all()` exactly like an unbounded one, and the file fires on every backend diff — so every diff produced a mandatory stop. Aligned with `ASYNC_AWAIT_REFLEX`'s own wording: **a match is not automatically a bug**, it is a place where the question must be answered; an *unanswered* hit is the stop.
- 🟠 **LD10 contradicted `CACHE_STRATEGY` while citing it.** "No invalidation path → no cache" versus that canon's "TTL-only — when the business allows eventual consistency and the risk is assessed". The reflex is later and more specific, so it would have won at the keyboard and rejected a design the cache canon permits. LD10 now asks for the TTL and the accepted staleness in writing instead.
- 🟠 **The per-trigger actions had quietly shrunk an existing minimum.** `ROLE_ARCH` STEP 0B demands five items unconditionally; the new rows gave money "Steps 1, 2, 5" and large lists "Steps 1, 3" — fewer. Stated explicitly: **these are floors on top of 0B, never reductions of it; where they differ the larger set wins.** Also: the real-time row routed *away* from Step 8, which is the real-time step, and the store/queue row omitted Step 1 while its own sentence said a store change without a load profile is a preference.
- 🟠 **`TC-12` still read the spine first**, contradicting model → load → spine in the one place @ARCH is told what to open. Reordered, with the order named as the rule.
- 🟠 **Block D reached no frontend role.** LD11 and LD12 are React checks and `ROLE_FRONTEND` activated only `MOTION_REFLEX`; `TC-01` likewise. A table screen — the exact surface those two checks exist for — never saw them. Wired into both. `TC-13` also gained the protocol: AI on a critical path is a load trigger and had neither.
- 🟠 **`ROLE_DEV`'s RECEIVES had no row for the load profile**, so LD1–LD12 asked "how many rows at ×5 growth" and "what is p95 here" about numbers @DEV was never handed. Added, with its own "if missing → stop and ask; do not invent a scale".
- Drift: `.cursorrules` version line · `RAG_CANON` §1.1's "full copy" of the reused-letter list was six letters stale · a stray `.md` line left in `SYSTEM_EVOLUTION_PROTOCOL`'s header · no mirror-index row for the LOAD FLOOR, now copied into five places.

**The completion rule, applied to this entry — `ROLE_LEO_EDITOR` §5, which the previous two entries did not use:**
```
CLAIM:   the load gate is reachable from a task that is not an architectural change
GREP:    rg -n "^1\.75|go to step 1\.75" roles/ROLE_LEAD.md
OUTPUT:  300:   NO → go to step 1.75      302:1.75 **A LOAD trigger?**
         (1.75 sits between 1.7 and 1.8 and is entered unconditionally from 1.7)

CLAIM:   an LD finding has a vector, a verdict and somewhere to be written
GREP:    rg -n "Vector 20|LD1|3\+ hits" roles/ROLE_QA_ARCH.md | head
OUTPUT:  Vector 20 present with the LD1-LD12 checklist, the five-that-are-red rule,
         the 3+ threshold, and "any unmet point → no 🟢"

CLAIM:   no dead paths introduced by this wave
GREP:    grep -rhoP 'roles/[A-Za-z0-9_/]+\.md' .cursorrules roles/ *.md | sort -u | while read x; do [ -f "$x" ] || echo BROKEN $x; done
OUTPUT:  (empty)
```

**v6.39c — `roles/LOAD_REFLEX.md`: the fourth leg, and the failure class whose symptom is success.**
The gate and the floor landed in v6.39b; this is what makes them reach the keyboard. Twelve literal greps @DEV runs over its own diff before handoff, mirrored at @QA_ARCH, reported even at zero.
- **The argument for it, stated in the file itself:** an async bug hangs, a race double-books — both announce themselves. **A load defect works perfectly**: on your machine, on staging, on the first two hundred rows. It is the only failure class here whose symptom is *success*, right up until the table grows, and by then it is in production inside a page a customer uses. No test fails. That is the whole case for greps rather than review.
- **Every check enforces one line of the LOAD FLOOR**, which is what couples the two: a floor with no reflex is a number in an artifact that never reaches the hand typing `.all()`.
- **Block A — unbounded:** `LD1` a query with no limit · `LD2` a list endpoint with no pagination *in its signature*, with a server max the caller cannot raise · `LD3` a column the UI sorts or filters by with no index created **in the same change** — cannot index it, then the UI does not offer it · `LD4` `count()` on a growing table for a UI element nobody reads.
- **Block B — multiplication:** `LD5` a query inside a loop (passes every test with a three-row fixture) · `LD6` the same N+1 one layer lower, in a serializer walking a relationship — *"a response shape is a load decision; this is where 'just add one field' becomes an outage"* · `LD7` fan-out with no ceiling, one request becoming N jobs.
- **Block C — the hot path:** `LD8` heavy synchronous work in a handler — *a report over 3s is a job, not a request* · `LD9` a pool size chosen per process rather than budgeted across API + workers + cron + migrations + a deploy overlap · `LD10` a hot uncached read, and the worse case, a cache with no invalidation path.
- **Block D — the client side:** `LD11` the server returns everything and the browser filters it · `LD12` a long list with no virtualisation and no page size the API enforces.
- **Scope is stated so the reflex layer does not overlap:** timeouts, the event loop, retries and queue liveness stay with `ASYNC_AWAIT_REFLEX`; locks and session guard rails with `DATABASE_RUNTIME_CANON`. This file owns **volume** — what has no ceiling, what multiplies, what does not belong in a request, and what the browser carries.
- **The question above all twelve**, and it is the floor's own stress test: *what does this do at ten times the current data?* Cannot answer it for the code you just wrote → you have not finished writing it.
- Routed into `TC-06` and `TC-07` as mandatory-over-the-diff (a reflex map, so it does not count against the six-file cap); wired into @DEV's activation and report line, and mirrored in @QA_ARCH's. `R` added to the constitution's list of reused detector letters.

**v6.39b — load gets its legs. The four-part test, applied to the second discipline.**
`ROLE_LEO_EDITOR` §4 was written as a general test — a discipline is enforceable only with a floor, detectors, a reflex and a blocking gate. Run against **system design under load**, it returned the same shape motion had: the knowledge was excellent and none of the four legs existed. `SYSTEM_DESIGN_PROTOCOL` held seven good steps, was routed, had a trigger table — and nothing blocked on it, nothing supplied a default, and the role that drafts the structure was also the role judging its behaviour under load.
- **THE LOAD FLOOR** — the "take this" that did not exist. Concurrency, RPS, table size, p95 targets and payload ceilings for an internal tool and for a public product, plus the five decisions the floor already makes so nobody re-decides them per module (every list paginated · every exposed filter column indexed · every outgoing call with a timeout and a named behaviour on timeout · anything past the write target becomes a queued job rather than a longer timeout · the connection budget summed across API, workers, cron and migrations). Written into the artifact marked `FLOOR — not measured` and the work continues. **No numbers from the business is no longer a reason to design for one user, and no longer a reason to wait.**
- **Growth ×10 as the floor's own stress test** — multiply and name what breaks first. A design never asked what breaks first has not been designed.
- **Triggers now carry ACTIONS.** Each row of the trigger table says which steps it produces and what specifically must be answered: money gets peak concurrency and behaviour under contention; real-time gets connection count as a capacity number; large lists get pagination, index coverage and the N+1 answer before the endpoint exists; AI/LLM gets provider latency as the budget and a named fallback. And `N/A` became a written answer with its reason, because *"no load analysis" and "the load analysis says this is fine" look identical afterwards, and only one of them is a decision.*
- **The blocking gate, and it fires in the right place.** @LEAD step 1.8 now checks the load trigger **before the spine**, not before DEV_PROMPTS. The order is stated in Law 31 and in @ARCH's own contract: **model → load → spine.** A spine drafted first answers its twelve vertebrae against a scale nobody chose.
- **@CREATOR supplies the input it always owned and never handed over.** Expected scale — user count, peak concentration, data growth — is a *product* fact living with @CREATOR, @BIZ and @DOMAIN_EXPERT, not with @ARCH. It goes in `PROJECT_PROFILE.md` §1 and in the package. @CREATOR does not run the protocol; it stops @ARCH having to guess, and an @ARCH guessing scale guesses one user.
- Routed into `TC-06` (backend-slice) ON DEMAND with the reason stated: a slice with money, a large list, search or real-time answers it *before the endpoint exists, not after it is slow*.
- **`ROLE_SYSTEM_EDITOR` renamed `ROLE_LEO_EDITOR`** (`@LEO_EDITOR`). The old name collided with *systems engineer* — a T3 naming failure introduced by the same wave that added the T3 test.

**v6.39 — the constitution is battle equipment. Everything else moves out.**
The question that drove this wave was not size, it was **what the machine does with what it reads on every task**. Text in `.cursorrules` is not a reference book — it is a *standing bias* mixed into every generation, in the most heavily weighted position of the context. Text behind the router is *retrieved knowledge* that arrives when relevant. Putting the second kind in the first place converts it into a bias it was never meant to be.
- **All instructions about how to edit a law left the constitution.** They fire on roughly one task in fifty, and their presence at the top of the always-loaded file had a specific cost that is not tokens: an agent doing routine work sees standing instructions about testing rules and auditing findings, and takes a licence to reason about the rules instead of doing the work. That has happened here more than once, including in this system's own recent history.
- **New: `roles/ROLE_LEO_EDITOR.md`** — the maintainer's role. **Not one of the 22**, not in the CHAIN, not in the ROLE MAP, not in `.cursorrules`: invoked by hand, in a fresh chat, between projects. It carries what the constitution should never have held — §1 **where a rule belongs at all** (three tests, the decisive one being *does this have to bias the agent before the agent knows it is relevant?* — that is why Law 25's seven steps stay and a procedure does not); §2 **what the machine does with the shape you write** (an exception list is read as a menu of escape hatches; an unfollowed pointer is a rule that does not exist; **a metric that measures the absence of a defect scores a dead page as perfect** — ask what a lazy implementation scores); §3 the router's two invariants; §4 the four-part enforceability test; §5 **the completion rule**: no completion claim without the grep that supports it, in a required CLAIM/GREP/OUTPUT block, because the author who diagnosed a problem correctly is the worst judge of whether the fix landed; §6 what not to do, including the two operations that have destroyed rules here before.
- **The registration rule in the constitution was rewritten into its battle form.** It described maintainer procedure — update the router in the same change, run the drift check. During delivery that is not actionable; what *is* actionable is: **a rule you cannot reach from the router is a finding to report, not a file to go and edit.**
- **Law 25 keeps all seven steps and loses the owner tags.** The steps stay in the constitution on the §1 test above: the failure they prevent is a step skipped by an agent who did not think it was doing design, and a canon arrives only after the class is already known. The per-step ownership tags were editor metadata — no trigger, no action, no stop condition — and are now one line at the top. ~700 characters of routine attention returned, behaviour unchanged.
- Checked while we were there: design and craft laws weigh **18,048** characters against **7,211** for backend and infrastructure. Design is not the underserved half of this constitution; it is two and a half times the heavier one.
