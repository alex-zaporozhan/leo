...
Series letters are reused across canons — `C`, `G`, `T`, `S`, `A`, `E`, `X`, `I`, `V`, `B`, `W`, `M` and `R` each mean different things in different files, and the list is **open, not closed**: assume any bare letter-number is ambiguous until the canon is named. (`M1` is "the bare fade" in the motion craft catalogue and a per-project moment ID in a motion-language passport; `I4` is "undo instead of confirm" in one canon and a hover distortion in another; `S1` is an identity surface in one and a horizontal scroll section in another; `V19` is a lint vector in one and a QA vector in another.) **Never cite a bare `C1` or `T3`: write the canon with it** (`FRONTEND_CAPABILITY C1`, `ARCH_SPINE T3`). An unqualified code is not a finding, it is a guess — and a guess that looks like a citation is worse than an open question, because the next reader treats it as checked.
...
**READ, in order:** 1) `roles/VISUAL_CRAFT_CANON.md` **§1–§6 · §9 · §11** (restraint, tonal depth, one light source, chroma, type scale, optics · the cheapness detector X1–X12 · **THE FLOOR** when there is no concept) · 2) `roles/INTERFACE_CRAFT_CANON.md` **§1 · §3 · §3.5 · §7** (the interaction inventory I1–I12 · data density · **the composition floor — what to BUILD on a list, form, record, filter bar and destructive action** · the stiffness detector ST1–ST12) · 3) `roles/LAYOUT_COMPOSITION.md` **§2 · §3 · §5** (primitive algebra · proximity as a number · action grammar) · 4) `roles/LAYOUT_INVARIANTS.md` **§1–§9 · §12** (deterministic geometry · collision and stacking) · 5) `roles/COMPONENT_REGISTRY.md` (whole — every block maps to a registered component, Law 25 step 4) · 6) `roles/MOTION_CRAFT_CANON.md` **§1 · §3** (**THE MOTION FLOOR** taken verbatim when no motion concept exists · the stiffness detector M1–M12) — and, at implementation time, `roles/MOTION_REFLEX.md` plus `roles/LOAD_REFLEX.md` **Block D** (LD11 the client fetching everything and filtering in memory · LD12 a long list with no virtualisation — the two checks that decide whether a table screen survives real data), which is a reflex map run over the diff rather than a canon read up front, and does not count against the cap. 
...
**ON DEMAND:** `roles/DATABASE_RUNTIME_CANON.md` · `roles/MIGRATIONS_PLAYBOOK.md` · `roles/CACHE_STRATEGY.md` · `roles/SYSTEM_DESIGN_PROTOCOL.md` (**the LOAD FLOOR and the load triggers — a slice with money, a large list, search or real-time has one, and it is answered before the endpoint exists, not after it is slow**) · `roles/LOAD_REFLEX.md` (**mandatory over the diff before handoff** — LD1–LD12; it is a reflex map run at implementation time, not a canon read up front, and does not count against the cap).

### TC-07 · async-pipeline — queues, workers, background jobs
**READ, in order:** 1) `roles/ASYNC_WORKERS_CANON.md` (three planes, laws AW-1…13, JOB and PIPELINE passports, crash-tests) · 2) `roles/ASYNC_AWAIT_REFLEX.md` (**mandatory before handoff**; `roles/LOAD_REFLEX.md` too where the pipeline reads or writes collections) · 3) `docs/artifacts/JOB_PASSPORTS_[PROJECT].md` / `PIPELINE_PASSPORT_[pipeline].md` — filled **before** code · 4) `roles/DATABASE_RUNTIME_CANON.md` (the lease, the corpse lock, SKIP LOCKED) · 5) `roles/DATA_INTEGRITY_CANON.md` (idempotency) · 6) `roles/LOGGING_OBSERVABILITY_PROTOCOL.md`.
...
**READ, in order** — and the order *is* the rule here (**model → load → spine**, Law 31): 1) `docs/artifacts/DOMAIN_MODEL_[MODULE].md` (structure is derived FROM the model — Law 42) · 2) `roles/SYSTEM_DESIGN_PROTOCOL.md` (load profile, latency budget, failure modes; **THE LOAD FLOOR** when the business has no figures) · 3) `roles/ARCH_SPINE_PROTOCOL.md` (twelve vertebrae as numbers, the complexity ladder — answered *against* the profile above, not against a scale nobody chose) · 4) `roles/DATA_STORE_SELECTION.md` · 5) `roles/STACK_SELECTION.md` · 6) `roles/ARCHITECTURE_DOCUMENTATION_STANDARD.md` (the ADR registry).
...
**READ, in order:** 1) `roles/ROLE_AI_ENGINEER.md` (the pillars, the passports, the EVAL_PLAN) · 2) `roles/RAG_ARCHITECTURE_STACK_2026.md` · 3) `docs/artifacts/RAG_PASSPORT.md` / `AGENT_GRAPH_PASSPORT.md` · 4) `docs/artifacts/EVAL_PLAN.md` (a release-blocking threshold as a number; "by eye in the chat" is a blocker) · 5) `roles/ASYNC_WORKERS_CANON.md` · 6) `roles/SYSTEM_DESIGN_PROTOCOL.md` (**AI/LLM on a critical path is a load trigger** — provider latency is the budget, its rate limit is the ceiling, and the fallback on provider failure is named before the feature exists; `roles/LOAD_REFLEX.md` at implementation time) · 7) `roles/DATA_INTEGRITY_CANON.md` (an agent with an external money/status effect).
...
**READ, in order:** 1) **`roles/ROLE_LEO_EDITOR.md`** (the maintainer's role — where a rule belongs at all, what the machine does with the shape you write, the router's two invariants, the four-part enforceability test, and the completion rule: **no completion claim without the grep that supports it**) · 2) **`roles/RULE_INTEGRITY_PROTOCOL.md`** (the seven tests a rule must pass, and the ladder between two true rules — run it on the proposed change **and on the finding that prompted it**) · 3) `roles/SYSTEM_EVOLUTION_PROTOCOL.md` (`@EVOLVE`) · 4) `roles/CONFLICT_REGISTRY.md` (does this change contradict a decided winner?) · 5) `roles/SYSTEM_FILES_MASTER.md` · 6) `roles/FILE_MAP.md` · 7) `roles/FRONTEND_CONSOLIDATION.md` (**before adding a rule: does it already live somewhere?** — the check whose absence is exactly how a duplicating canon gets written; it does not count against the cap because TC-20 is maintenance, not delivery).
