...
roles/ROLE_LEO_EDITOR.md            ← the MAINTAINER's role: where a rule belongs, what the machine does with a rule's shape, route invariants, the four-part enforceability test, the completion rule. Manual invocation only; not one of the 22, not in the chain
roles/LOAD_REFLEX.md                   ← the load reflex: LD1–LD12 literal greps over the diff — unbounded queries, N+1, fan-out, heavy work in the request path, pool budget, client-side filtering. Enforces the LOAD FLOOR
